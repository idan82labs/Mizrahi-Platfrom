...
def process_manager(manager_name, manager_code, funds_list_path, token, args, output_base_dir):
    ...
    cmd = [
        sys.executable,
        str(script_dir / "mizrahi_special_transactions.py"),
        "--mutual-funds-list",
        str(funds_list_path),
        "--input-report",
        str(report_path),
...
"""
    ...
    if gmail_user and gmail_password:
        ...
        log("To send email manually, use:")
        log(
            f"  python send_email.py --batch-dir {output_dir} --gmail-user YOUR_EMAIL --gmail-app-password YOUR_PASSWORD"
        )
        log("=" * 60)
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./output/hook2"),
        help="Base output directory (default: ./output/hook2)",
    )
    parser.add_argument(
        "--skip-tase-prices",
        action="store_true",
    ...
    parser.add_argument(
        "--send-email",
        action="store_true",
        help="Send email after processing (requires GMAIL_USER and GMAIL_APP_PASSWORD env vars or args)",
    )
...
def main():
    ...
    # Get Gmail credentials from args or environment
    ...
    if args.managers:
        ...
        managers_to_process = {
            name: FUND_MANAGERS[name] for name in manager_names if name in FUND_MANAGERS
        }
    else:
